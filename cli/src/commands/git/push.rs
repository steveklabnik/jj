...
/// commit. Any descendant commits beyond the bookmark are not pushed. For
///
/// If the local bookmark has diverged from the remote (non-fast-forward), the
/// push will update the remote bookmark to the new position after passing
/// safety checks. This is similar to `git push --force-with-lease` - the remote
/// is updated only if its current state matches what Jujutsu last fetched.
///
