...
export default function remarkStripMdExtension() {
  return (tree, file) => {
    ...
    const relativePath = contentDocsMatch ? contentDocsMatch[1] : '';
    const currentDir = relativePath ? path.dirname(relativePath) : '';
    ...
    const dirParts = currentDir === '.' || currentDir === '' ? [] : currentDir.split(path.sep);
    ...
    visit(tree, ['link', 'definition'], (node) => {
      ...
      if (pathWithoutMd.startsWith('/')) {
        // Absolute link: convert to relative using ../ × effectiveDepth
      ...
      // Clean up any Windows-style backslashes (for cross-platform compatibility)
